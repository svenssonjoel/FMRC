/* 
   This file contains a modified version of the code with the below license statement. 
*/
...
static void i2c_delay(i2c_bb_state *s);
...
static bool clock_stretch_timeout(i2c_bb_state *s) {
  uint32_t time_start = gptGetCounterX(s->gpt); //timer_time_now();

  while(READ_SCL() == 0) {
    if (abs(gptGetCounterX(s->gpt) - time_start) > 10000) { // 0.01
      s->has_error = true;
      return false;
    }
  }
